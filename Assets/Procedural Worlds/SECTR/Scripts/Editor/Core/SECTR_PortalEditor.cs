...
public class SECTR_PortalEditor : SECTR_HullEditor 
{
	...
	public override void OnEnable()
	{
        ...
        SceneView.duringSceneGui += this.HandleSceneGUI;
#else
        SceneView.onSceneGUIDelegate += this.HandleSceneGUI;
#endif
        base.OnEnable();
	...
	void _DrawViewportGUI(SECTR_Portal myPortal)
	{
		...
		else if(Selection.gameObjects.Length == 1)
		{
            ...
            if (GUILayout.Button(new GUIContent(myPortal.HullMesh ? "Redraw Portal" : "Draw Portal", "Lets you (re)create the geometry for this Portal"), buttonStyle))
            {
                string sceneDir;
                string sceneName;
                if (!SECTR_Asset.GetCurrentSceneParts(out sceneDir, out sceneName))
                {
                    if (EditorUtility.DisplayDialog("Scene needs to be saved first", "This scene needs to be saved first before you can create portal data, would you like to save your scene now?", "Yes", "Cancel"))
                    {
                    ...
                    {
                        Debug.LogWarning("Scene not saved when trying to create portal, aborting...");
                    }
                }
                else
                {
